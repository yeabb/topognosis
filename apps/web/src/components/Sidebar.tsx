...
export default function Sidebar({ activeGraphId, onSelectGraph, onNewGraph }: Props) {
  ...
  return (
    <aside className="flex flex-col h-full w-[260px] shrink-0 bg-[#171717]">
      {/* New graph button */}
      ...
      <div className="flex-1 overflow-y-auto px-3 pb-3">
        {loading ? (
          ...
          <div className="flex flex-col gap-4">
            {groups.map((group) => (
              <div key={group.label}>
                <p className="text-neutral-600 text-xs font-medium px-3 py-1">{group.label}</p>
      ...
      {/* Footer — user avatar + name + sign out */}
